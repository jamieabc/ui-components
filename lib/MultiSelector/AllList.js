...
var AllList = function (_Component) {
  ...
  function AllList(props) {
    ...
    _this.getOut = function (e) {
      e.preventDefault();
      _this.handleQueryChange({ parent_id: null, offset: 0, keyword: null });
    };
      ...
      if (parentId) {
        if (_this.isCheckedAll()) {
          ...
          if (selectedIds.includes(parentId)) {
            selectedItems.forEach(function (item) {
              if (item.id == parentId) {
                item.children = (0, _uniqBy2.default)(item.children.concat(listingItems), 'id');
              }
            });
          } else {
            ...
            selectedItems = (0, _uniqBy2.default)(selectedItems.concat([currentItem]), 'id');
          }
        }
      } else {
        if (_this.isCheckedAll()) {
          ...
          selectedItems = (0, _uniqBy2.default)(selectedItems.concat(listingItems), 'id');
        }
      }
      _this.props.onChange(selectedItems);
    };

    _this.handleQueryChange = function (newQuery) {
      var query = void 0;
      if (newQuery && (newQuery.hasOwnProperty('offset') || newQuery.hasOwnProperty('order'))) {
        query = Object.assign({}, _this.props.dataTableProps.query, newQuery);
      } else {
        query = Object.assign({}, _this.props.dataTableProps.query, newQuery, { offset: 0 });
      }
      _this.props.dataTableProps.onQueryChange(query);
    };

    _this.isCheckedAll = function () {
      if ((0, _isEmpty2.default)(_this.props.listingItems)) {
        return false;
      }
      var selectedIds = [];
      var parentId = _this.props.dataTableProps.query.parent_id;
    ...
    _this.isChecked = function (ls, id) {
      var ids = ls.map(function (item) {
      ...
      return (0, _flatten2.default)(ids).includes(id);
    };
    ...
    _this.getSelectorColumn = function () {
      return [{
        ...
        render: function render(value, data) {
          var checked = _this.isChecked(_this.props.allSelectedItems, data.id);

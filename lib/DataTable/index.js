...
var DataTable = function (_Component) {
  ...
  function DataTable(props) {
    ...
    _this.handleSortChange = function (sortInfo) {
      ...
      var nextSortInfo = (0, _last2.default)(sortInfo) || preSortInfo;
      if (sortInfo.length === 0) {
        // resort the same column and dir is asc
        nextSortInfo.dir = 'asc';
      } else {
        if (preSortInfo.name !== nextSortInfo.name) {
          // sort not equal name, then sort dir default desc
          nextSortInfo.dir = 'desc';
        }
      }
      _this.handlePageChange({ order: _Helpers2.default.stringifySort([nextSortInfo]) });
      // Send out new sort info
      if (_this.props.onStatusChange) {
        _this.props.onStatusChange({ sort: nextSortInfo });
      }
    };
    ...
    _this.handlePageChange = function (newQuery) {
      ...
      _this.props.onPageChange(query);
    };

    _this.onColumnResize = function (firstCol, firstSize) {
      var columnMinWidth = firstCol.minWidth || _this.props.columnMinWidth;
  ...
  _createClass(DataTable, [{
    ...
    value: function componentWillReceiveProps(nextProps) {
      ...
      if ((0, _isEqual2.default)(oldColumnNames, newColumnNames)) {
        this.columns = this.columns.map(function (oldColumn) {
          var newColumn = (0, _find2.default)(nextProps.columns, function (column) {
    ...
    value: function render() {
      ...
      var rowHeight = function () {
        var _rowHeight = Number.parseInt(_this2.props.rowHeight, 10);
        return (0, _isInteger2.default)(_rowHeight) ? _rowHeight : DEFAULT_ROW_HEIGHT;
      }();
...
DataTable.propTypes = {
  ...
  focusable: _react.PropTypes.bool, // Set to true to selected the column
